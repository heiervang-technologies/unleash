...
- Project is a Rust-based CLI/TUI tool named "agent-unleashed", wrapping Anthropic's Claude Code.
- It provides a `tmux` based headless mode, auto-mode, plugin support, and version management.
- The `autx` command (headless mode) has command injection vulnerabilities in `src/tmux.rs` when joining shell arguments.
...
- **Tmux Integration**: `autx` relies heavily on parsing and waiting for output files to stabilize rather than programmatic IPC.
...
- **P1: High-Impact Bugs & Failures**
  - **Command Injection in `autx start` and `autx send` (src/tmux.rs):** User input `args` are joined by spaces without proper shell escaping, meaning an argument like `hello; rm -rf /` gets injected into the tmux shell. (Effort: S) - Fix by properly escaping or using array-based execution in tmux.
