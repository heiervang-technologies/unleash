...
```!
"${CLAUDE_PLUGIN_ROOT}/scripts/cu-wait" --setup
```

After setup, the omnihook is ready to receive messages via `cu-queue`.
...
3. **Instant Wakeup**: When `cu-queue --notify` is called, any waiting `cu-wait` unblocks immediately
...
3. If Claude is in `cu-wait`, it wakes up immediately
