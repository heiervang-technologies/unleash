#!/usr/bin/env bash
# check-enabled.sh — exit 0 if hyprland-focus is enabled in unleash config,
...
# guard prevents the hooks from firing on every Stop / UserPromptSubmit.
#
# Empty enabled_plugins list = "all enabled" (backwards-compat semantics).

set -uo pipefail

PLUGIN_NAME="${1:-hyprland-focus}"
