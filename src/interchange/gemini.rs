...
mod tests {
    ...
    fn test_tool_call_with_result_round_trip() {
        ...
        // Verify tool call is in assistant message (hub[2]) and
        // tool result is extracted to a separate user message (hub[3])
        if let HubRecord::Message(ref hub_msg) = hub[2] {
            ...
            // ToolResults are extracted to a subsequent user message
        ...
        // Verify the extracted tool result is in the subsequent user message
        if let HubRecord::Message(ref result_msg) = hub[3] {
            ...
            panic!("Expected hub[3] to be a Message with extracted ToolResult");
        }
