#!/usr/bin/env bash
# Test script for cu auth command

set -e

CU_BIN="${CU_BIN:-./target/release/cu}"

echo "Testing cu auth command..."
echo "================================"
echo

# Test 1: Basic auth check
echo "Test 1: Basic auth check"
if $CU_BIN auth > /dev/null 2>&1; then
...
$CU_BIN auth --verbose | head -5
echo "✓ Verbose output works"
echo

# Test 3: JSON output
echo "Test 3: JSON output"
JSON_OUTPUT=$($CU_BIN auth --json)
echo "$JSON_OUTPUT"
...
JSON_VERBOSE=$($CU_BIN auth --json --verbose)
echo "$JSON_VERBOSE"
