...
| **voice-output** | Text-to-speech for agent responses (uses `say` command) | Requires `say` in PATH |
...
| **supercompact** | Entity-aware context compaction using the EITF algorithm | `/supercompact` slash command |
...
Routes agent text responses through the `say` TTS command. Supports
VibeVoice, OpenAI, and ElevenLabs backends depending on your `say`
configuration.
...
Compresses conversation context using the Entity-Indexed Token Fusion (EITF)
algorithm. Roughly 400x faster than `/compact` with 2x better entity
retention. Invoke with the `/supercompact` slash command.
